...
class CodegenContext extends Logging {
   ...
   * evaluation.
   */
  private def collectSubExprCodes(
  ...
  def subexpressionElimination(expressions: Seq[Expression]): SubExprCodes = {
    ...
    val subExprCode = splitExpressionsWithCurrentInputs(
      collectSubExprCodes(subExprsMap.values), "subexprFunc_split")
    SubExprCodes(subExprsMap.toMap, subExprCode, exprCodes.flatten)
  }
  ...
  def generateExpressions(
    ...
    if (doSubexpressionElimination) {
      ...
      (generatedExprs, subExprs.subExprCode)
    } else {
      (cleanedExpressions.map(e => e.genCode(this)), "")
    }
  }
