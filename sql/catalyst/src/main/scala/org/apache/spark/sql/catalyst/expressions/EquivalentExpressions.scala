...
class EquivalentExpressions {
   ...
   */
  def addExpr(expr: Expression, conditional: Boolean = false): Boolean = {
  ...
  private def findCommonExprs(exprs: Seq[Expression]): Seq[ExpressionEquals] = {
    ...
    }.map { case (commonExpr, _) => commonExpr }.toSeq
  }
  ...
  private def childrenToRecurse(expr: Expression): RecurseChildren = expr match {
    ...
    case i: If => RecurseChildren(Seq(i.predicate), Seq(i.trueValue, i.falseValue),
      Seq(i.trueValue, i.falseValue))
    case c: CaseWhen =>
