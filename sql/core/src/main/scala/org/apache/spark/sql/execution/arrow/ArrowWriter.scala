...
object ArrowWriter {
  ...
  def create(schema: StructType, timeZoneId: String, useLargeVarTypes: Boolean = false
      ): ArrowWriter = {
    val arrowSchema = ArrowUtils.toArrowSchema(schema, timeZoneId, useLargeVarTypes)
